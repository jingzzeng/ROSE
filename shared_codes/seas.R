seas <- function(x = NULL, y = NULL, yclass = NULL, d = NULL, categorical=FALSE, H=5, M = NULL, U = NULL, nobs = NULL, nlambda=NULL, lambda.factor=NULL, lam1 = NULL, lam2 = NULL, gamma = NULL, lam1_fac=seq(1.0,0.3, length.out = 10), lam2_fac=seq(0.01,0.3, length.out = 10), eps = 1e-3, maxit = 1e+3, warning.it = 1, original_y = FALSE, ...){
  if(is.null(M) || is.null(U)){
    ...
    if(is.null(lam1) || is.null(lam2)){
      fit_1 <- cv.msda_seas(x, y, yclass = yclass, nlambda=nlambda, lambda.factor=lambda.factor, nfolds = 5, maxit=1e3, warning.it = warning.it, categorical = categorical, original_y = original_y)
      M <- fit_1$M
      U <- fit_1$U
      M_fold <- fit_1$M_fold
      U_fold <- fit_1$U_fold
  ...
  if(is.vector(lam1) && (length(lam1) == 1) && (lam1 == 0) && is.vector(lam2) && (length(lam2) == 1) && (lam2 == 0)){
    ...
    fit <- admm_seas(M, U, nobs, nvars, lam1, lam2, gamma, eps, maxit, d, warning.it = warning.it, ...)
    B_l <- fit$B
    beta_l <- fit$beta
    if (all(sapply(beta_l, is.null))){
      ...
      if(warning.it >= 2) warning("In \"seas\": No converged results returned.\n")
      if(length(B_l) == 1){
        B_l <- B_l[[1]]
        beta_l <- beta_l[[1]]
        rank_l <- NA
...
cv.seas <- function(x, y, yclass = NULL, d = NULL, categorical=FALSE, H=5, lambda.factor=NULL, nlambda=NULL, nfolds = 5, fold = NULL, lam1 = NULL, lam2 = NULL, gamma = NULL, lam1_fac=seq(1.0,0.3, length.out = 10), lam2_fac=seq(0.01,0.3, length.out = 10), plot = FALSE, eps = 1e-3, maxit = 1e+3, trace.it = TRUE, solution.path = FALSE, warning.it = 1, original_y = FALSE, ...){
  ...
  if(is.null(lam1) || is.null(lam2)){
    fit_1 <- cv.msda_seas(x, y, yclass = yclass, nlambda=nlambda, lambda.factor=lambda.factor, fold = fold, maxit=1e3, warning.it =  warning.it, plot = plot, categorical = categorical, original_y = original_y)
    M <- fit_1$M
    U <- fit_1$U
    M_fold <- fit_1$M_fold
    U_fold <- fit_1$U_fold
  ...
  out_all <- lapply(1:nfolds, function(k){
    ...
    if(is.null(M_fold) || is.null(U_fold)){
      ...
      fit_fold <- seas(x_train, y_train, yclass = yclass_train, lam1 = lam1, lam2 = lam2, gamma = gamma, eps = eps, maxit = maxit, d = d, categorical = categorical, ...)
    }
    else{
      fit_fold <- seas(M = M_fold[[k]], U = U_fold[[k]], nobs = sum(fold!=k), lam1 = lam1, lam2 = lam2, gamma = gamma, eps = eps, maxit = maxit, d = d, categorical = categorical, ...)
    }

    err <- 0
    if(fit_fold$code != 0){
      err <- 1
      if(warning.it >= 2) warning(paste0("Fold ", k, ": No converged results returned from the seas function."))
    ...
    eval_fold <- eval_dc_seas(beta_l, x_val, y_val, H = H, categorical = categorical, original_y = original_y)
  ...
  if(solution.path){
    if(is.null(M) || is.null(U)){
      # Fit with seas function
      fit <- seas(x, y, yclass = yclass, lam1 = lam1, lam2 = lam2, gamma = gamma, eps = eps, maxit = maxit, d = d, categorical = categorical, ...)
    }
    else{
      fit <- seas(M = M, U = U, nobs = NROW(x), lam1 = lam1, lam2 = lam2, gamma = gamma, eps = eps, maxit = maxit, d = d, categorical = categorical, ...)
    }
    # beta_l <- fit$beta
    # rank_l <- fit$rank
    # s_l <- fit$s
  ...
  # Fit with seas function
  if(is.null(M) || is.null(U)){
    fit <- seas(x, y, yclass = yclass, lam1 = lam1_min, lam2 = lam2_min, gamma = gamma_min, eps = eps, maxit = maxit, d = d, warning.it =  warning.it, categorical = categorical, ...)
  }
  else{
    fit <- seas(M = M, U = U, nobs = NROW(x), lam1 = lam1_min, lam2 = lam2_min, gamma = gamma_min, eps = eps, maxit = maxit, d = d, warning.it =  warning.it, categorical = categorical, ...)
  }
...
msda_seas <- function(x, y, yclass=NULL, categorical=FALSE, H=5, lambda.factor=NULL, nlambda=NULL, lambda=NULL, dfmax=NULL, pmax=NULL, pf=NULL, M = NULL, U = NULL, nobs=NULL, nclass=NULL, eps=1e-04, maxit=1e+06, sml=1e-06, warning.it=0, perturb=NULL){
...
cv.msda_seas <- function(x, y, yclass=NULL, categorical=FALSE, H=5, lambda.factor=NULL, nlambda=100, nfolds=5, fold = NULL, lambda=NULL, maxit=1e3, warning.it = 1, plot=FALSE, original_y = FALSE){
  if(is.data.frame(x)) x <- as.matrix(x)
  if(is.null(yclass)){
    if(categorical == FALSE){
      ybreaks <- as.numeric(quantile(y, probs=seq(0,1, by=1/H), na.rm=TRUE))
  ...
  fit <- msda_seas(x, y, yclass = yclass, lambda.factor = lambda.factor, nlambda = nlambda, lambda = lambda, maxit=maxit, warning.it = warning.it, categorical = categorical)
  lambda <- fit$lambda
  beta_l <- fit$theta
  M <- fit$M
  U <- fit$U
  ...
  cv_out <- lapply(1:nfolds, function(k){
    ...
    fit_fold <- msda_seas(x_train, y_train, yclass_train, lambda.factor=lambda.factor, nlambda=nlambda, lambda = lambda, maxit=maxit, warning.it = warning.it, categorical = categorical)
    M_fold <- fit_fold$M
    U_fold <- fit_fold$U
    ...
    eval_fold <- eval_dc_seas(beta_fold, x_val, y_val, H = H, categorical = categorical, original_y = original_y)
...
MU_seas <- function(x, y, yclass=NULL, categorical = FALSE, H = 5){
  if(is.null(yclass)){
    if(categorical == FALSE){
      ybreaks <- as.numeric(quantile(y, probs=seq(0,1, by=1/H), na.rm=TRUE))
      yclass <- cut(y, breaks = ybreaks, include.lowest = TRUE, labels = FALSE)
...
admm_seas <- function(M, U, nobs, nvars, lam1, lam2, gam, eps=1e-3, maxit=1e+3, d = NULL, warning.it = 0, ...){
    ...
    for(j in 1:n3){
      ...
      for(k in 1:n2){
        ...
        repeat{
          ...
          out_B <- updateB_seas(M, U, lambda1, opts)
          Bnew <- out_B$Bnew
          jerr <- out_B$jerr
          if(jerr != 0) break
          
          # Update C
          Cnew <- updateC_seas(Bnew, lambda2, gamma, etaold)
...
updateB_seas <- function(M, U, lambda1, opts){
  U <- t(U)
...
updateC_seas <- function(Bnew, lambda2, gamma, etaold){
...
eval_dc_seas <- function(Beta, x, y, H = H, categorical = FALSE, original_y = FALSE){
  if(categorical == FALSE){
      ybreaks <- as.numeric(quantile(y, probs=seq(0,1, by=1/H), na.rm=TRUE))
      yclass <- cut(y, breaks = ybreaks, include.lowest = TRUE, labels = FALSE)
  }
  ...
  for(i in seq_along(Beta)){
    if(is.null(Beta[[i]])){
      ...
      # If exists infinity in the response
      tryCatch({
        if(original_y){
          result[i] <- 1 - dcor(x %*% mat, y)
        }else{
          result[i] <- 1 - dcor(x %*% mat, ydummy)
        }
      },
      error = function(e){
        result[i] <- NA
